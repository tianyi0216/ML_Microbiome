...
class Graph:
	...
	def build_graph(self, mapfile='phyloT_generated_tree_1711651391_newick.txt'):
		self.node_count = 0
		my_map = []
		my_list=[]
		layer = -1
	...
	def populate_graph(self, lab, x):
		...
		for i in range(0, len(list(lab.index))):
			...
			# 	level = 'kingdom'
			print(lab.index[i])
			node = self.get_node_by_name(lab.index[i])
			if node == None:
				node = self.get_node_by_name(lab.index[i])
	...
	def prune_graph(self, features_df):
		...
		for i in range(0, len(list(features.index))):
			...
			while found == False:
				#for level in levels:
				if features.index[i] != "NA":
					node = self.get_node_by_name(features.index[i])

					if node == None:
						node = Node(features.index[i])
						node.set_abundance(0)
